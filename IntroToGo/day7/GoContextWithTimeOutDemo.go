...
func main() {
	...
	//method2: to generate a context along with a timeout time
	//the derived context get automatically cancelled when the timeout time reaches
	context1, cancelfn := context.WithTimeout(contextVar, 3*time.Second)
	defer cancelfn()
