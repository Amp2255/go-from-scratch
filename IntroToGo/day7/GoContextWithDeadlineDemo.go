...
func main() {
	...
	//method2: to generate a context along with a deadline
	//the derived context get automatically cancelled when the deadline reaches
	context1, cancelfn := context.WithDeadline(contextVar, time.Now().Add(4*time.Second))
	defer cancelfn()
